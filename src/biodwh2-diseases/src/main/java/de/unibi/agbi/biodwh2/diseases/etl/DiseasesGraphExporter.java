...
public class DiseasesGraphExporter extends GraphExporter<DiseasesDataSource> {
    ...
    //Main Output, returns true if export was successful, false otherwise, but real export logic is the graph object with nodes and edges
    @Override
    protected boolean exportGraph(final Workspace workspace, final Graph graph) throws ExporterException {
        //defines schemaic constraints in the graph "rule" still no data just labels
        graph.addIndex(IndexDescription.forNode(GENE_LABEL, ID_KEY, IndexDescription.Type.UNIQUE)); //ID_KEY in GraphExporter ID_KEY = "id"
        graph.addIndex(IndexDescription.forNode(DISEASE_LABEL, ID_KEY, IndexDescription.Type.UNIQUE));

        //generate nodes with exportGenes and exportDiseases 
        ...
        //generate edges with exportEdges
    ...
    private Map<String, Long> exportGenes(final Workspace workspace, final Graph graph) {
        ...
        final Map<String, String> genes = new HashMap<>(); //HashMap<>() is a java structure similar to a dictionary in python
        
        // process every entry(row) in each file looking for 1 and 2 column as geneIdentifier and geneName 
        parseTsvFile(workspace, KnowledgeEntry.class, DiseasesUpdater.KNOWLEDGE_FILTERED_FILE_NAME,
                     (entry) -> updateEntityEntry(genes, entry.geneIdentifier, entry.geneName)); // lambda expression
        ...
        for (final String id : genes.keySet()) {
            // id not always ENSP identifier, exception in files rRNAs and pseudogenes 
            final String name = genes.get(id);
            final Node node;
            if (id.equals(name))
            // e.g 18S_rRNA	 18S_rRNA	DOID:9643	Babesiosis  id:18S_rRNA
                node = graph.addNode(GENE_LABEL, ID_KEY, id);
            else
            // e.g ENSP00000001146	CYP26B1	 DOID:2340	 Craniosynostosis  id:ENSP00000001146 name:CYP26B1
    ...
    // parse the files and extract the gene and disease identifier and name 
